...
- The stack is running and `migrate` has completed, so the schema is at `0009_audit_booking_deleted`.
...
revision is not `0009_audit_booking_deleted`, stop and find out why before backing up.
...
Expect `0009_audit_booking_deleted` — the same revision as the source, with no upgrade or
...
Expect **22** — the 21 application tables plus `alembic_version`. The
